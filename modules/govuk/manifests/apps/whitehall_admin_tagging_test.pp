...
class govuk::apps::whitehall_admin_tagging_test(
  ...
  govuk::app { 'whitehall-admin-tagging-test':
    app_type               => 'rack',
    vhost                  => $vhost,
    port                   => $port,
  ...
  if $configure_admin == true {
    ...
    govuk::app::nginx_vhost { 'whitehall-admin-tagging-test-admin':
      vhost                 => "whitehall-admin-tagging-test-admin.${app_domain}",
      app_port              => $port,
      protected             => true,
      deny_framing          => true,
      asset_pipeline        => true,
      ...
      proxy_set_header X-Accel-Mapping /data/uploads/whitehall-admin-tagging-test/clean/=/clean/;
      ...
      location ~ /clean/(.*) {
        ...
        alias /data/uploads/whitehall-admin-tagging-test/clean/$1;
      }
      ...
      location /government/uploads/uploaded {
        ...
        alias /data/uploads/whitehall-admin-tagging-test/clean/uploaded;
    ...
    govuk::logstream { 'whitehall_admin_tagging_test_scheduled_publishing_json_log':
      logfile => '/var/apps/whitehall-admin-tagging-test/log/production_scheduled_publishing.json.log',
      fields  => {'application' => 'whitehall-admin-tagging-test'},
      json    => true,
    }

    govuk::logstream { 'whitehall_admin_tagging_test_sidekiq_json_log':
      logfile => '/var/apps/whitehall-admin-tagging-test/log/sidekiq.json.log',
      fields  => {'application' => 'whitehall-sidekiq'},
      json    => true,
    }

    govuk::procfile::worker { 'whitehall-admin-tagging-test-admin':
      setenv_as      => 'whitehall',
    ...
    govuk::app::envvar {
      "${title}-RACK_MULTIPART_PART_LIMIT":
          app     => 'whitehall-admin-tagging-test',
    ...
    govuk::app::envvar {
      "${title}-GOVUK_ASSET_ROOT":
        ...
        value   => "//whitehall-admin-tagging-test-admin.${app_domain}";
    }
  }
}
